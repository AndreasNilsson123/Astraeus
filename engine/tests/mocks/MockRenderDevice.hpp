...
public:
    ...
    uint32_t create_buffer(size_t size, const void* data) override {
        uint32_t id = next_id_++;
        operations_.push_back({Operation::CREATE_BUFFER, "size=" + std::to_string(size), id});
        return id;
    }
    
    void delete_buffer(uint32_t buffer_id) override {
        operations_.push_back({Operation::DELETE_BUFFER, "", buffer_id});
    }
    
    uint32_t create_texture(uint32_t width, uint32_t height, TextureFormat format) override {
        uint32_t id = next_id_++;
    ...
    void delete_texture(uint32_t texture_id) override {
