#ifndef ASTRAEUS_LIT_MESH_PASS_HPP
#define ASTRAEUS_LIT_MESH_PASS_HPP
...
/**
 * LitMeshPass: Renders meshes with lighting (Lambert + Blinn-Phong).
 * Uses LitMaterial for realistic lighting with directional light.
 */
class LitMeshPass : public RenderPass {
public:
    inline LitMeshPass(AssetManager* asset_manager, MaterialLibrary* material_library);
    ~LitMeshPass() override = default;
    ...
    inline const char* get_name() const override { return "LitMesh"; }

private:
    inline void compute_mvp_matrix(const Transform& transform, const Camera& camera, 
                                   uint32_t width, uint32_t height, float* out_mvp);
    inline void compute_model_matrix(const Transform& transform, float* out_model);
    inline void compute_normal_matrix(const float* model, float* out_normal_matrix);
...
inline LitMeshPass::LitMeshPass(AssetManager* asset_manager, MaterialLibrary* material_library)
    : gl_device_(nullptr)
...
inline bool LitMeshPass::initialize(RenderDevice* device) {
    ...
    if (!material_library_) {
        std::cerr << "[LitMeshPass] Material library not provided" << std::endl;
        return false;
    }

    std::cout << "[LitMeshPass] Initialized" << std::endl;
    return true;
}

inline void LitMeshPass::execute(RenderDevice* device, World* world) {
    (void)device;
    ...
    gl_device_->push_debug_group("LitMeshPass");

    // Get lit material
    LitMaterial* lit_material = material_library_->get_default_lit();
    if (!lit_material) {
        std::cerr << "[LitMeshPass] No lit material available" << std::endl;
        gl_device_->pop_debug_group();
        return;
    }

    // Get lighting parameters from world
    ...
    // Set light parameters on material
    ...
    // Bind material once (all meshes use the same lit material)
    lit_material->bind(device);
    ...
    for (uint32_t entity_id : entities) {
        ...
        float model[16];
        compute_model_matrix(*transform, model);
        
        float normal_matrix[9];
        compute_normal_matrix(model, normal_matrix);
        ...
        lit_material->set_parameter("mvp", mvp_param);
        
        // Set model matrix parameter
        MaterialParameter model_param;
        model_param.type = MaterialParameterType::Mat4;
        std::memcpy(model_param.data.mat4_value, model, sizeof(float) * 16);
        lit_material->set_parameter("model", model_param);
        
        // Set view position parameter (for specular)
        MaterialParameter view_pos_param;
        view_pos_param.type = MaterialParameterType::Vec3;
        view_pos_param.data.vec3_value[0] = eye_x;
        view_pos_param.data.vec3_value[1] = eye_y;
        view_pos_param.data.vec3_value[2] = eye_z;
        lit_material->set_parameter("viewPos", view_pos_param);
        ...
        lit_material->set_parameter("baseColor", color_param);

        // Apply all parameters
        lit_material->apply_parameters(device);
...
inline void LitMeshPass::on_resize(uint32_t width, uint32_t height) {
    viewport_width_ = width;
    viewport_height_ = height;
}

// Matrix math helpers (same as MeshPass)

inline void LitMeshPass::identity_matrix(float* out) {
    for (int i = 0; i < 16; i++) {
        out[i] = (i % 5 == 0) ? 1.0f : 0.0f;
    }
}

inline void LitMeshPass::translation_matrix(float x, float y, float z, float* out) {
    identity_matrix(out);
    out[12] = x;
    out[13] = y;
    out[14] = z;
...
inline void LitMeshPass::scale_matrix(float sx, float sy, float sz, float* out) {
    identity_matrix(out);
    out[0] = sx;
    out[5] = sy;
    out[10] = sz;
...
inline void LitMeshPass::perspective_matrix(float fov, float aspect, float near_dist, float far_dist, float* out) {
...
inline void LitMeshPass::look_at_matrix(float eye_x, float eye_y, float eye_z,
                                    float center_x, float center_y, float center_z,
                                    float up_x, float up_y, float up_z, float* out) {
    // Forward vector
...
inline void LitMeshPass::multiply_matrices(const float* a, const float* b, float* out) {
    float temp[16];
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            temp[i*4 + j] = 0;
...
inline void LitMeshPass::compute_model_matrix(const Transform& transform, float* out_model) {
    // Model matrix (translation and scale)
    float translation[16];
    translation_matrix(transform.pos_x, transform.pos_y, transform.pos_z, translation);
    
    float scale[16];
    scale_matrix(transform.scale_x, transform.scale_y, transform.scale_z, scale);
    
    multiply_matrices(translation, scale, out_model);
}

inline void LitMeshPass::compute_normal_matrix(const float* model, float* out_normal_matrix) {
    // Extract upper-left 3x3 from model matrix (ignoring translation)
    // This is a simplified version; proper implementation would compute inverse transpose
    // For uniform scaling, the 3x3 part is sufficient
    out_normal_matrix[0] = model[0];  out_normal_matrix[1] = model[1];  out_normal_matrix[2] = model[2];
    out_normal_matrix[3] = model[4];  out_normal_matrix[4] = model[5];  out_normal_matrix[5] = model[6];
    out_normal_matrix[6] = model[8];  out_normal_matrix[7] = model[9];  out_normal_matrix[8] = model[10];
}

inline void LitMeshPass::compute_mvp_matrix(const Transform& transform, const Camera& camera,
                                        uint32_t width, uint32_t height, float* out_mvp) {
    // Model matrix
    float model[16];
    compute_model_matrix(transform, model);
    ...
    multiply_matrices(vp, model, out_mvp);
}

} // namespace astraeus

#endif // ASTRAEUS_LIT_MESH_PASS_HPP
