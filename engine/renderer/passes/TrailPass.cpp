...
static const char* trail_vertex_shader = R"(
#version 330 core
...
static const char* trail_fragment_shader = R"(
#version 330 core
in vec4 vColor;
...
bool TrailPass::initialize(RenderDevice* device) {
...
void TrailPass::create_geometry() {
    // Create VAO
...
void TrailPass::destroy_geometry() {
    if (gl_device_) {
        if (vao_ != 0) {
...
void TrailPass::update_trail_data(World* world) {
    if (!world) {
        vertex_count_ = 0;
        return;
    }
...
void TrailPass::execute(RenderDevice* device, World* world) {
    (void)device;
...
void TrailPass::on_resize(uint32_t width, uint32_t height) {
