#include "DeterministicSimGenerator.hpp"
#include <cmath>
#include <cstring>
#include <iostream>
#include <random>
...
DeterministicSimGenerator::DeterministicSimGenerator()
...
bool DeterministicSimGenerator::initialize(uint32_t entity_count, uint32_t seed) {
    if (is_initialized_) {
        return true;
    }
...
void DeterministicSimGenerator::shutdown() {
    if (!is_initialized_) {
        return;
    }
...
void DeterministicSimGenerator::generate_default_entities(uint32_t count, uint32_t seed) {
    entity_configs_.clear();
...
void DeterministicSimGenerator::add_entity_config(const SimEntityConfig& config) {
    entity_configs_.push_back(config);
}

void DeterministicSimGenerator::compute_entity_state(
...
std::vector<uint8_t> DeterministicSimGenerator::generate_frame(double timestamp) {
    if (!is_initialized_) {
