...
bool SnapshotStore::initialize(uint32_t max_entities) {
    if (is_initialized_) {
        return true;
    }
...
bool SnapshotStore::begin_write(double timestamp, uint64_t frame_number) {
    if (!is_initialized_) {
        return false;
    }
...
void SnapshotStore::write_entity(const EntitySnapshot& entity, const EntityMetadata& metadata) {
    if (!is_writing_) {
...
SnapshotView SnapshotStore::get_latest_snapshot() const {
    if (!is_initialized_) {
        return SnapshotView();
    }
